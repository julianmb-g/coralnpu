...
class TileLinkULInterface:
    ...
    async def _host_a_driver(self, prefix, timeout=4096):
        ...
        while True:
            ...
            a_valid.value = 1
            for prop in [
                    "opcode", "param", "size", "source", "address", "mask",
                    "data"
            ]:
        ...
        while True:
            ...
            try:
                if d_valid.value and d_ready.value == 1:
                    ...
                    txn = {'user': {}}
                    for prop in [
                            "opcode", "param", "size", "source", "sink",
                            "data", "error"
                    ]:
                        txn[prop] = getattr(self.dut,
                                            f"{prefix}_d_bits_{prop}").value
                    ...
                    for field in user_fields:
                        ...
                        if hasattr(self.dut, signal_name):
                            txn["user"][field] = getattr(
                                self.dut, signal_name).value

                    await self.host_d_fifo.put(txn)
            except Exception as e:
                try:
                    is_reset = (self.reset.value
                                == 0) if self.reset_active_low else (
                                    self.reset.value == 1)
                except Exception:
                    is_reset = True
                if is_reset:
        ...
        while True:
            ...
            try:
                if a_valid.value and a_ready.value == 1:
                    txn = {"user": {}}
                    for prop in [
                            "opcode", "param", "size", "source", "address",
                            "mask", "data"
                    ]:
                        txn[prop] = getattr(self.dut,
                                            f"{prefix}_a_bits_{prop}").value
                    user_fields = [
    ...
    async def _device_d_driver(self, prefix, timeout=4096):
        ...
        while True:
            ...
            d_valid.value = 1
            for prop in [
                    "opcode", "param", "size", "source", "sink", "data",
                    "error"
            ]:
