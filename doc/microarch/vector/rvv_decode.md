...
When enabled, `RvvCore.scala` generates the `RVV_CONFIG_SVH` header with the macro define:

```verilog
`define ZVFBFWMA_ON
```

If disabled, the define is commented out:
...
The instruction decoder in Chisel (`RvvDecode.scala`) decodes BFloat16 opcodes unconditionally. However, the backend SystemVerilog decoder (`rvv_backend_decode_unit_ari.sv` / `rvv_backend_decode_unit_ari_de2.sv`) uses conditional logic gated by `ZVFBFWMA_ON` to validate and process these opcodes. When enabled, the decoder maps the following instructions:
...
**Provenance & Traceability** - **Verified As Of:** 2026-07-03 - **Upstream Commit:** f5f6c88d3dff8cb198cd89420919b6863667f3e0 - **Primary Source(s):** `hdl/chisel/src/coralnpu/rvv/RvvDecode.scala`, `hdl/verilog/rvv/design/rvv_backend_falu_unit.sv`, `hdl/chisel/src/coralnpu/rvv/RvvCore.scala`, `hdl/verilog/rvv/design/rvv_backend_decode_unit_ari.sv`, `hdl/verilog/rvv/design/rvv_backend_decode_unit_ari_de2.sv` - **Disclaimer:** AI-generated/assisted; RTL is the source of truth.
