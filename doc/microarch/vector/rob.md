...
- **Writeback Ports (Update):** The ROB can receive up to **4 uop results** per cycle from the various execution PUs (`NUM_SMPORT`).
...
Under the current implementation, vector exception discard policies are enforced. As a critical architectural design constraint:
...
## RVV 1.0 exception routing violation

**Spec Violation:** The CoralNPU vector core implementation currently violates the RISC-V Vector 1.0 specification regarding exception routing. Vector-initiated floating-point exceptions (e.g., division by zero, overflow) generated in the VFPU are silently discarded and are not routed to the architectural `fcsr` (`fflags`). Consequently, software cannot observe vector floating-point exceptions.

**Design Requirement:** To comply with ADR-172 and the RISC-V specification, future implementations MUST enable correct fflags propagation from the ROB retirement ports through the `RvvCoreIO` boundary to correctly update the architectural `fcsr`.

--------------------------------------------------------------------------------

**Provenance & Traceability** - **Verified As Of:** 2026-08-03 - **Upstream Commit:** [1126ed3fa244b38ee06fa002a5c640df9dec36f4](https://github.com/google/coralnpu/commit/1126ed3fa244b38ee06fa002a5c640df9dec36f4) - **Primary Source(s):** `hdl/verilog/rvv/design/rvv_backend_rob.sv` - **Disclaimer:** AI-generated/assisted; RTL is the source of truth.
