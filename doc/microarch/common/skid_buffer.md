...
| :--- | :--- | :--- |
| `data` | Input | Read data payload. |
| `id` | Input | Transaction ID. Used to route data to `ibus` (1) or `ebus` (0). |
| `resp` | Input | Read response status (Okay, ExOkay, SlvErr, DecErr). |
| `last` | Input | Indicates the last transfer in a burst. |
| `valid` | Input | Indicates that valid read data is available. |
| `ready` | Output | Indicates that the buffer can accept read data. |

--------------------------------------------------------------------------------

**Provenance & Traceability** - **Verified As Of:** 2026-08-03 - **Upstream Commit:** [1126ed3fa244b38ee06fa002a5c640df9dec36f4](https://github.com/google/coralnpu/commit/1126ed3fa244b38ee06fa002a5c640df9dec36f4) - **Primary Source(s):** `hdl/chisel/src/coralnpu/CoreAxi.scala:L254-260` - **Disclaimer:** AI-generated/assisted; RTL is the source of truth.
