...
- **Sigmoid / Tanh**: Handled via polynomial approximation or Lookup Tables leveraging the [VFPU](../microarch/vector/vfpu.md).
...
2. **Lookup Tables**: For higher performance, software can pre-calculate activation values into a table stored in [TCM](../microarch/memory/tcm.md). The `VRGATHER` instruction can then be used to perform high-speed vector register-to-register permutations.
...
| Sigmoid | VRGATHER    | PMT/RDT | 0.125 - 0.25 instr / cycle |
