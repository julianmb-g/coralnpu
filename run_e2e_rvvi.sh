...
if [ "$USE_PODMAN" -eq 1 ]; then
  ...
  podman run --userns=keep-id:uid=1000,gid=1000 --pids-limit=-1 -it --rm -v $PWD:$PWD -w $PWD -v /tmp/bazel_cache:/home/builder/.cache localhost/coralnpu bash -c "set -o pipefail; bazel build //examples:coralnpu_v2_rvv_add_intrinsic && cp bazel-bin/examples/coralnpu_v2_rvv_add_intrinsic.elf \$PWD/test.elf && bazel run //tests/verilator_sim:core_rvvi_sim -- --cycles=50000 --rvvi_out=\$PWD/trace.rvvi \$PWD/test.elf 2>&1 | tee \$PWD/tmp_log/rvvi_sim.log || exit 1"
else
  ...
  bazel run //tests/verilator_sim:core_rvvi_sim -- --cycles=50000 --rvvi_out="$PWD/trace.rvvi" "$PWD/test.elf" 2>&1 | tee "$PWD/tmp_log/rvvi_sim.log" || exit 1
fi
