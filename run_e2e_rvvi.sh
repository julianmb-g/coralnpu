...
if [ "$USE_PODMAN" -eq 1 ]; then
  ...
  podman run --userns=keep-id:uid=1000,gid=1000 --pids-limit=-1 -it --rm -v $PWD:$PWD -w $PWD -v $HOME/.cache/bazel:/home/builder/.cache localhost/coralnpu bash -c "set -o pipefail; bazel build //examples:coralnpu_v2_rvv_add_intrinsic && cp bazel-bin/examples/coralnpu_v2_rvv_add_intrinsic.elf /tmp/test.elf && bazel run //tests/verilator_sim:core_rvvi_traced_sim -- --instructions=50000 --rvvi_out=\$PWD/trace.rvvi /tmp/test.elf 2>&1 | tee \$PWD/tmp_log/rvvi_sim.log || exit 1"
  podman run --userns=keep-id:uid=1000,gid=1000 --pids-limit=-1 -it --rm -v $PWD:$PWD -w $PWD -v $HOME/.cache/bazel:/home/builder/.cache localhost/coralnpu bash -c "set -o pipefail; bazel build //examples:coralnpu_v2_hello_world_add_floats && cp bazel-bin/examples/coralnpu_v2_hello_world_add_floats.elf /tmp/float_test.elf && bazel run //tests/verilator_sim:core_rvvi_traced_sim -- --instructions=50000 --rvvi_out=\$PWD/trace_float.rvvi /tmp/float_test.elf 2>&1 | tee \$PWD/tmp_log/rvvi_float_sim.log || exit 1"
else
