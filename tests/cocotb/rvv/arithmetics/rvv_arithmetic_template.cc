...
void{MATH_OP} _{OP_SUFFIX}(const {SCALAR_TYPE} * in_buf_1,
                           ...
                           {SCALAR_TYPE} * out_buf) {
  {VEC_TYPE} input_v1 =
      __riscv_vle{SEW} _v_{OP_SUFFIX}(in_buf_1, {NUM_OPERANDS});
  ...
  {MATH_OP} _result = __riscv_v{MATH_OP} _vv_{OP_SUFFIX}(
      vd_orig, input_v1, input_v2, {NUM_OPERANDS});
#elif defined(TEST_UNARY)
  {
    VEC_TYPE
  }
  {MATH_OP} _result =
      __riscv_v{MATH_OP} _v_{OP_SUFFIX}(input_v1, {NUM_OPERANDS});
#else
  {
    VEC_TYPE
  }
