...
ABSL_FLAG(bool, backdoor_load, false,
          "Enable high-speed backdoor code loading");

static bool run(absl::string_view name, absl::string_view binary,
                const int cycles, const bool trace, const bool debug_axi,
                const bool instr_trace, const bool backdoor_load) {
  absl::Mutex halted_mtx;
  absl::CondVar halted_cv;
  CoreMiniAxiTb tb(std::string(name).c_str(), cycles, /* random= */ false,
                   debug_axi, instr_trace, backdoor_load,
                   /*wfi_cb=*/std::nullopt,
  ...
  // This allows Verilog `initial` blocks to run before we try to interact with
  // the DUT model.
...
extern "C" int sc_main(int argc, char **argv) {
  ...
  return run(SyscTb::GetName(argv[0]), absl::GetFlag(FLAGS_binary),
             ...
             absl::GetFlag(FLAGS_backdoor_load))
             ? 0
             : 1;
}
