...
// Fulfills the RVVI Traced Target Implementation requirement. Extracts state
// from io_debug_rb_... ports and formats it using the asynchronous trace
// daemon.
...
ABSL_FLAG(bool, csv, false,
          "Output RVVI trace in Comma-Separated Values format");
...
#define DECLARE_RB_INST(x)                                                     \
  ...
  sc_in<sc_bv<KP_rvvRegCountWidth>>                                            \
...
#undef DECLARE_RB_INST
  ...
  mpact::sim::riscv::rvvi::SpscRingBuffer<mpact::sim::riscv::rvvi::TracePacket,
                                          mpact::sim::riscv::rvvi::kBufferSize>
      *buffer;
  bool e_sent = false;
  ...
  void PushPacket(const mpact::sim::riscv::rvvi::TracePacket &packet,
                  const char *label) {
    if (e_sent)
      return;
  ...
  CoreRvviTb(sc_module_name name, int instruction_limit, uint64_t cycle_limit,
             bool random,
             mpact::sim::riscv::rvvi::SpscRingBuffer<
                 mpact::sim::riscv::rvvi::TracePacket,
                 mpact::sim::riscv::rvvi::kBufferSize> *buf,
             mpact::sim::util::ElfProgramLoader *elf_loader)
      : SyscTb(name, instruction_limit * 10, random), elf_loader(elf_loader),
        buffer(buf), instruction_limit(instruction_limit),
  ...
  if (io_debug_rb_inst_##x##_valid.read()) {                                   \
    ...
    if (is_vector) {                                                           \
      for (int v_i = 0; v_i < 8; ++v_i) {                                      \
        ...
        switch (v_i) {                                                         \
        case 0:                                                                \
          v_valid = io_debug_rb_inst_##x##_bits_vecWrites_0_valid.read();      \
          if (v_valid) {                                                       \
            v_data = io_debug_rb_inst_##x##_bits_vecWrites_0_bits_data.read(); \
            v_idx = io_debug_rb_inst_##x##_bits_vecWrites_0_bits_idx.read()    \
                        .to_uint();                                            \
          }                                                                    \
          break;                                                               \
        case 1:                                                                \
          v_valid = io_debug_rb_inst_##x##_bits_vecWrites_1_valid.read();      \
          if (v_valid) {                                                       \
            v_data = io_debug_rb_inst_##x##_bits_vecWrites_1_bits_data.read(); \
            v_idx = io_debug_rb_inst_##x##_bits_vecWrites_1_bits_idx.read()    \
                        .to_uint();                                            \
          }                                                                    \
          break;                                                               \
        case 2:                                                                \
          v_valid = io_debug_rb_inst_##x##_bits_vecWrites_2_valid.read();      \
          if (v_valid) {                                                       \
            v_data = io_debug_rb_inst_##x##_bits_vecWrites_2_bits_data.read(); \
            v_idx = io_debug_rb_inst_##x##_bits_vecWrites_2_bits_idx.read()    \
                        .to_uint();                                            \
          }                                                                    \
          break;                                                               \
        case 3:                                                                \
          v_valid = io_debug_rb_inst_##x##_bits_vecWrites_3_valid.read();      \
          if (v_valid) {                                                       \
            v_data = io_debug_rb_inst_##x##_bits_vecWrites_3_bits_data.read(); \
            v_idx = io_debug_rb_inst_##x##_bits_vecWrites_3_bits_idx.read()    \
                        .to_uint();                                            \
          }                                                                    \
          break;                                                               \
        case 4:                                                                \
          v_valid = io_debug_rb_inst_##x##_bits_vecWrites_4_valid.read();      \
          if (v_valid) {                                                       \
            v_data = io_debug_rb_inst_##x##_bits_vecWrites_4_bits_data.read(); \
            v_idx = io_debug_rb_inst_##x##_bits_vecWrites_4_bits_idx.read()    \
                        .to_uint();                                            \
          }                                                                    \
          break;                                                               \
        case 5:                                                                \
          v_valid = io_debug_rb_inst_##x##_bits_vecWrites_5_valid.read();      \
          if (v_valid) {                                                       \
            v_data = io_debug_rb_inst_##x##_bits_vecWrites_5_bits_data.read(); \
            v_idx = io_debug_rb_inst_##x##_bits_vecWrites_5_bits_idx.read()    \
                        .to_uint();                                            \
          }                                                                    \
          break;                                                               \
        case 6:                                                                \
          v_valid = io_debug_rb_inst_##x##_bits_vecWrites_6_valid.read();      \
          if (v_valid) {                                                       \
            v_data = io_debug_rb_inst_##x##_bits_vecWrites_6_bits_data.read(); \
            v_idx = io_debug_rb_inst_##x##_bits_vecWrites_6_bits_idx.read()    \
                        .to_uint();                                            \
          }                                                                    \
          break;                                                               \
        case 7:                                                                \
          v_valid = io_debug_rb_inst_##x##_bits_vecWrites_7_valid.read();      \
          if (v_valid) {                                                       \
            v_data = io_debug_rb_inst_##x##_bits_vecWrites_7_bits_data.read(); \
            v_idx = io_debug_rb_inst_##x##_bits_vecWrites_7_bits_idx.read()    \
                        .to_uint();                                            \
          }                                                                    \
          break;                                                               \
        }                                                                      \
        if (v_valid) {                                                         \
          ...
          for (int sp = 0; sp < sub_packets; ++sp) {                           \
            ...
            vpacket.chunk_size =                                               \
                std::min<int>(32, (KP_rvvVlen / 8) - sp * 32);                 \
            ...
            for (int w = 0; w < 4; ++w) {                                      \
              ...
              if (word_idx_0 < (KP_rvvVlen / 32))                              \
                val64 |= v_data.get_word(word_idx_0);                          \
              if (word_idx_1 < (KP_rvvVlen / 32))                              \
                val64 |= (static_cast<uint64_t>(v_data.get_word(word_idx_1))   \
                          << 32);                                              \
    ...
    bool writes_rd = (opcode == 0x13) || (opcode == 0x33) ||                   \
                     (opcode == 0x37) || (opcode == 0x17) ||                   \
    ...
    if (writes_rd) {                                                           \
      ...
      if (rd != 0 || (wb_idx >= 32 && wb_idx < 64)) {                          \
        ...
        rpacket.raw_words[0] =                                                 \
            io_debug_rb_inst_##x##_bits_data.read().to_uint64();               \
...
#undef COUNT_RETIRE
    ...
    if ((io_halted.read() || io_fault.read() || ebreak_halt || mpause_halt ||
         instruction_count >= instruction_limit) &&
        !e_sent) {
...
static int CoreRvviRun(absl::string_view name, absl::string_view bin,
  ...
  mpact::sim::riscv::rvvi::SpscRingBuffer<mpact::sim::riscv::rvvi::TracePacket,
                                          mpact::sim::riscv::rvvi::kBufferSize>
      buffer;
  CoreRvviTb testbench("CoreRvviTb", instruction_limit, cycle_limit,
                       /* random= */ false, &buffer, &elf_loader);
#else
  CoreRvviTb testbench("CoreRvviTb", instruction_limit, cycle_limit,
                       /* random= */ false, nullptr, &elf_loader);
#endif

#if TRACE_ENABLED
  std::ofstream trace_stream((std::string(rvvi_out)));

  mpact::sim::riscv::rvvi::TraceDaemon<KP_rvvVlen,
                                       mpact::sim::riscv::rvvi::kBufferSize>
      daemon(&buffer, &trace_stream, &elf_loader);
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_0_bits_vecWrites_0_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_0_bits_vecWrites_1_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_0_bits_vecWrites_2_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_0_bits_vecWrites_3_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_0_bits_vecWrites_4_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_0_bits_vecWrites_5_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_0_bits_vecWrites_6_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_0_bits_vecWrites_7_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_1_bits_vecWrites_0_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_1_bits_vecWrites_1_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_1_bits_vecWrites_2_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_1_bits_vecWrites_3_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_1_bits_vecWrites_4_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_1_bits_vecWrites_5_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_1_bits_vecWrites_6_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_1_bits_vecWrites_7_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_2_bits_vecWrites_0_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_2_bits_vecWrites_1_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_2_bits_vecWrites_2_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_2_bits_vecWrites_3_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_2_bits_vecWrites_4_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_2_bits_vecWrites_5_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_2_bits_vecWrites_6_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_2_bits_vecWrites_7_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_3_bits_vecWrites_0_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_3_bits_vecWrites_1_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_3_bits_vecWrites_2_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_3_bits_vecWrites_3_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_3_bits_vecWrites_4_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_3_bits_vecWrites_5_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_3_bits_vecWrites_6_bits_idx;
  ...
  sc_signal<sc_bv<KP_rvvRegCountWidth>>
      io_debug_rb_inst_3_bits_vecWrites_7_bits_idx;
...
#define DECLARE_RETIRE_PORT(x)                                                 \
  ...
  sc_signal<sc_bv<KP_retirementBufferIdxWidth>>                                \
      io_debug_rb_inst_##x##_bits_idx;                                         \
...
#undef DECLARE_CSR_IN
  ...
  testbench.io_debug_rb_inst_0_bits_vecWrites_0_valid(
      io_debug_rb_inst_0_bits_vecWrites_0_valid);
  testbench.io_debug_rb_inst_0_bits_vecWrites_0_bits_data(
      io_debug_rb_inst_0_bits_vecWrites_0_bits_data);
  testbench.io_debug_rb_inst_0_bits_vecWrites_0_bits_idx(
      io_debug_rb_inst_0_bits_vecWrites_0_bits_idx);
  testbench.io_debug_rb_inst_0_bits_vecWrites_1_valid(
      io_debug_rb_inst_0_bits_vecWrites_1_valid);
  testbench.io_debug_rb_inst_0_bits_vecWrites_1_bits_data(
      io_debug_rb_inst_0_bits_vecWrites_1_bits_data);
  testbench.io_debug_rb_inst_0_bits_vecWrites_1_bits_idx(
      io_debug_rb_inst_0_bits_vecWrites_1_bits_idx);
  testbench.io_debug_rb_inst_0_bits_vecWrites_2_valid(
      io_debug_rb_inst_0_bits_vecWrites_2_valid);
  testbench.io_debug_rb_inst_0_bits_vecWrites_2_bits_data(
      io_debug_rb_inst_0_bits_vecWrites_2_bits_data);
  testbench.io_debug_rb_inst_0_bits_vecWrites_2_bits_idx(
      io_debug_rb_inst_0_bits_vecWrites_2_bits_idx);
  testbench.io_debug_rb_inst_0_bits_vecWrites_3_valid(
      io_debug_rb_inst_0_bits_vecWrites_3_valid);
  testbench.io_debug_rb_inst_0_bits_vecWrites_3_bits_data(
      io_debug_rb_inst_0_bits_vecWrites_3_bits_data);
  testbench.io_debug_rb_inst_0_bits_vecWrites_3_bits_idx(
      io_debug_rb_inst_0_bits_vecWrites_3_bits_idx);
  testbench.io_debug_rb_inst_0_bits_vecWrites_4_valid(
      io_debug_rb_inst_0_bits_vecWrites_4_valid);
  testbench.io_debug_rb_inst_0_bits_vecWrites_4_bits_data(
      io_debug_rb_inst_0_bits_vecWrites_4_bits_data);
  testbench.io_debug_rb_inst_0_bits_vecWrites_4_bits_idx(
      io_debug_rb_inst_0_bits_vecWrites_4_bits_idx);
  testbench.io_debug_rb_inst_0_bits_vecWrites_5_valid(
      io_debug_rb_inst_0_bits_vecWrites_5_valid);
  testbench.io_debug_rb_inst_0_bits_vecWrites_5_bits_data(
      io_debug_rb_inst_0_bits_vecWrites_5_bits_data);
  testbench.io_debug_rb_inst_0_bits_vecWrites_5_bits_idx(
      io_debug_rb_inst_0_bits_vecWrites_5_bits_idx);
  testbench.io_debug_rb_inst_0_bits_vecWrites_6_valid(
      io_debug_rb_inst_0_bits_vecWrites_6_valid);
  testbench.io_debug_rb_inst_0_bits_vecWrites_6_bits_data(
      io_debug_rb_inst_0_bits_vecWrites_6_bits_data);
  testbench.io_debug_rb_inst_0_bits_vecWrites_6_bits_idx(
      io_debug_rb_inst_0_bits_vecWrites_6_bits_idx);
  testbench.io_debug_rb_inst_0_bits_vecWrites_7_valid(
      io_debug_rb_inst_0_bits_vecWrites_7_valid);
  testbench.io_debug_rb_inst_0_bits_vecWrites_7_bits_data(
      io_debug_rb_inst_0_bits_vecWrites_7_bits_data);
  testbench.io_debug_rb_inst_0_bits_vecWrites_7_bits_idx(
      io_debug_rb_inst_0_bits_vecWrites_7_bits_idx);
  ...
  testbench.io_debug_rb_inst_1_bits_vecWrites_0_valid(
      io_debug_rb_inst_1_bits_vecWrites_0_valid);
  testbench.io_debug_rb_inst_1_bits_vecWrites_0_bits_data(
      io_debug_rb_inst_1_bits_vecWrites_0_bits_data);
  testbench.io_debug_rb_inst_1_bits_vecWrites_0_bits_idx(
      io_debug_rb_inst_1_bits_vecWrites_0_bits_idx);
  testbench.io_debug_rb_inst_1_bits_vecWrites_1_valid(
      io_debug_rb_inst_1_bits_vecWrites_1_valid);
  testbench.io_debug_rb_inst_1_bits_vecWrites_1_bits_data(
      io_debug_rb_inst_1_bits_vecWrites_1_bits_data);
  testbench.io_debug_rb_inst_1_bits_vecWrites_1_bits_idx(
      io_debug_rb_inst_1_bits_vecWrites_1_bits_idx);
  testbench.io_debug_rb_inst_1_bits_vecWrites_2_valid(
      io_debug_rb_inst_1_bits_vecWrites_2_valid);
  testbench.io_debug_rb_inst_1_bits_vecWrites_2_bits_data(
      io_debug_rb_inst_1_bits_vecWrites_2_bits_data);
  testbench.io_debug_rb_inst_1_bits_vecWrites_2_bits_idx(
      io_debug_rb_inst_1_bits_vecWrites_2_bits_idx);
  testbench.io_debug_rb_inst_1_bits_vecWrites_3_valid(
      io_debug_rb_inst_1_bits_vecWrites_3_valid);
  testbench.io_debug_rb_inst_1_bits_vecWrites_3_bits_data(
      io_debug_rb_inst_1_bits_vecWrites_3_bits_data);
  testbench.io_debug_rb_inst_1_bits_vecWrites_3_bits_idx(
      io_debug_rb_inst_1_bits_vecWrites_3_bits_idx);
  testbench.io_debug_rb_inst_1_bits_vecWrites_4_valid(
      io_debug_rb_inst_1_bits_vecWrites_4_valid);
  testbench.io_debug_rb_inst_1_bits_vecWrites_4_bits_data(
      io_debug_rb_inst_1_bits_vecWrites_4_bits_data);
  testbench.io_debug_rb_inst_1_bits_vecWrites_4_bits_idx(
      io_debug_rb_inst_1_bits_vecWrites_4_bits_idx);
  testbench.io_debug_rb_inst_1_bits_vecWrites_5_valid(
      io_debug_rb_inst_1_bits_vecWrites_5_valid);
  testbench.io_debug_rb_inst_1_bits_vecWrites_5_bits_data(
      io_debug_rb_inst_1_bits_vecWrites_5_bits_data);
  testbench.io_debug_rb_inst_1_bits_vecWrites_5_bits_idx(
      io_debug_rb_inst_1_bits_vecWrites_5_bits_idx);
  testbench.io_debug_rb_inst_1_bits_vecWrites_6_valid(
      io_debug_rb_inst_1_bits_vecWrites_6_valid);
  testbench.io_debug_rb_inst_1_bits_vecWrites_6_bits_data(
      io_debug_rb_inst_1_bits_vecWrites_6_bits_data);
  testbench.io_debug_rb_inst_1_bits_vecWrites_6_bits_idx(
      io_debug_rb_inst_1_bits_vecWrites_6_bits_idx);
  testbench.io_debug_rb_inst_1_bits_vecWrites_7_valid(
      io_debug_rb_inst_1_bits_vecWrites_7_valid);
  testbench.io_debug_rb_inst_1_bits_vecWrites_7_bits_data(
      io_debug_rb_inst_1_bits_vecWrites_7_bits_data);
  testbench.io_debug_rb_inst_1_bits_vecWrites_7_bits_idx(
      io_debug_rb_inst_1_bits_vecWrites_7_bits_idx);
  ...
  testbench.io_debug_rb_inst_2_bits_vecWrites_0_valid(
      io_debug_rb_inst_2_bits_vecWrites_0_valid);
  testbench.io_debug_rb_inst_2_bits_vecWrites_0_bits_data(
      io_debug_rb_inst_2_bits_vecWrites_0_bits_data);
  testbench.io_debug_rb_inst_2_bits_vecWrites_0_bits_idx(
      io_debug_rb_inst_2_bits_vecWrites_0_bits_idx);
  testbench.io_debug_rb_inst_2_bits_vecWrites_1_valid(
      io_debug_rb_inst_2_bits_vecWrites_1_valid);
  testbench.io_debug_rb_inst_2_bits_vecWrites_1_bits_data(
      io_debug_rb_inst_2_bits_vecWrites_1_bits_data);
  testbench.io_debug_rb_inst_2_bits_vecWrites_1_bits_idx(
      io_debug_rb_inst_2_bits_vecWrites_1_bits_idx);
  testbench.io_debug_rb_inst_2_bits_vecWrites_2_valid(
      io_debug_rb_inst_2_bits_vecWrites_2_valid);
  testbench.io_debug_rb_inst_2_bits_vecWrites_2_bits_data(
      io_debug_rb_inst_2_bits_vecWrites_2_bits_data);
  testbench.io_debug_rb_inst_2_bits_vecWrites_2_bits_idx(
      io_debug_rb_inst_2_bits_vecWrites_2_bits_idx);
  testbench.io_debug_rb_inst_2_bits_vecWrites_3_valid(
      io_debug_rb_inst_2_bits_vecWrites_3_valid);
  testbench.io_debug_rb_inst_2_bits_vecWrites_3_bits_data(
      io_debug_rb_inst_2_bits_vecWrites_3_bits_data);
  testbench.io_debug_rb_inst_2_bits_vecWrites_3_bits_idx(
      io_debug_rb_inst_2_bits_vecWrites_3_bits_idx);
  testbench.io_debug_rb_inst_2_bits_vecWrites_4_valid(
      io_debug_rb_inst_2_bits_vecWrites_4_valid);
  testbench.io_debug_rb_inst_2_bits_vecWrites_4_bits_data(
      io_debug_rb_inst_2_bits_vecWrites_4_bits_data);
  testbench.io_debug_rb_inst_2_bits_vecWrites_4_bits_idx(
      io_debug_rb_inst_2_bits_vecWrites_4_bits_idx);
  testbench.io_debug_rb_inst_2_bits_vecWrites_5_valid(
      io_debug_rb_inst_2_bits_vecWrites_5_valid);
  testbench.io_debug_rb_inst_2_bits_vecWrites_5_bits_data(
      io_debug_rb_inst_2_bits_vecWrites_5_bits_data);
  testbench.io_debug_rb_inst_2_bits_vecWrites_5_bits_idx(
      io_debug_rb_inst_2_bits_vecWrites_5_bits_idx);
  testbench.io_debug_rb_inst_2_bits_vecWrites_6_valid(
      io_debug_rb_inst_2_bits_vecWrites_6_valid);
  testbench.io_debug_rb_inst_2_bits_vecWrites_6_bits_data(
      io_debug_rb_inst_2_bits_vecWrites_6_bits_data);
  testbench.io_debug_rb_inst_2_bits_vecWrites_6_bits_idx(
      io_debug_rb_inst_2_bits_vecWrites_6_bits_idx);
  testbench.io_debug_rb_inst_2_bits_vecWrites_7_valid(
      io_debug_rb_inst_2_bits_vecWrites_7_valid);
  testbench.io_debug_rb_inst_2_bits_vecWrites_7_bits_data(
      io_debug_rb_inst_2_bits_vecWrites_7_bits_data);
  testbench.io_debug_rb_inst_2_bits_vecWrites_7_bits_idx(
      io_debug_rb_inst_2_bits_vecWrites_7_bits_idx);
  ...
  testbench.io_debug_rb_inst_3_bits_vecWrites_0_valid(
      io_debug_rb_inst_3_bits_vecWrites_0_valid);
  testbench.io_debug_rb_inst_3_bits_vecWrites_0_bits_data(
      io_debug_rb_inst_3_bits_vecWrites_0_bits_data);
  testbench.io_debug_rb_inst_3_bits_vecWrites_0_bits_idx(
      io_debug_rb_inst_3_bits_vecWrites_0_bits_idx);
  testbench.io_debug_rb_inst_3_bits_vecWrites_1_valid(
      io_debug_rb_inst_3_bits_vecWrites_1_valid);
  testbench.io_debug_rb_inst_3_bits_vecWrites_1_bits_data(
      io_debug_rb_inst_3_bits_vecWrites_1_bits_data);
  testbench.io_debug_rb_inst_3_bits_vecWrites_1_bits_idx(
      io_debug_rb_inst_3_bits_vecWrites_1_bits_idx);
  testbench.io_debug_rb_inst_3_bits_vecWrites_2_valid(
      io_debug_rb_inst_3_bits_vecWrites_2_valid);
  testbench.io_debug_rb_inst_3_bits_vecWrites_2_bits_data(
      io_debug_rb_inst_3_bits_vecWrites_2_bits_data);
  testbench.io_debug_rb_inst_3_bits_vecWrites_2_bits_idx(
      io_debug_rb_inst_3_bits_vecWrites_2_bits_idx);
  testbench.io_debug_rb_inst_3_bits_vecWrites_3_valid(
      io_debug_rb_inst_3_bits_vecWrites_3_valid);
  testbench.io_debug_rb_inst_3_bits_vecWrites_3_bits_data(
      io_debug_rb_inst_3_bits_vecWrites_3_bits_data);
  testbench.io_debug_rb_inst_3_bits_vecWrites_3_bits_idx(
      io_debug_rb_inst_3_bits_vecWrites_3_bits_idx);
  testbench.io_debug_rb_inst_3_bits_vecWrites_4_valid(
      io_debug_rb_inst_3_bits_vecWrites_4_valid);
  testbench.io_debug_rb_inst_3_bits_vecWrites_4_bits_data(
      io_debug_rb_inst_3_bits_vecWrites_4_bits_data);
  testbench.io_debug_rb_inst_3_bits_vecWrites_4_bits_idx(
      io_debug_rb_inst_3_bits_vecWrites_4_bits_idx);
  testbench.io_debug_rb_inst_3_bits_vecWrites_5_valid(
      io_debug_rb_inst_3_bits_vecWrites_5_valid);
  testbench.io_debug_rb_inst_3_bits_vecWrites_5_bits_data(
      io_debug_rb_inst_3_bits_vecWrites_5_bits_data);
  testbench.io_debug_rb_inst_3_bits_vecWrites_5_bits_idx(
      io_debug_rb_inst_3_bits_vecWrites_5_bits_idx);
  testbench.io_debug_rb_inst_3_bits_vecWrites_6_valid(
      io_debug_rb_inst_3_bits_vecWrites_6_valid);
  testbench.io_debug_rb_inst_3_bits_vecWrites_6_bits_data(
      io_debug_rb_inst_3_bits_vecWrites_6_bits_data);
  testbench.io_debug_rb_inst_3_bits_vecWrites_6_bits_idx(
      io_debug_rb_inst_3_bits_vecWrites_6_bits_idx);
  testbench.io_debug_rb_inst_3_bits_vecWrites_7_valid(
      io_debug_rb_inst_3_bits_vecWrites_7_valid);
  testbench.io_debug_rb_inst_3_bits_vecWrites_7_bits_data(
      io_debug_rb_inst_3_bits_vecWrites_7_bits_data);
  testbench.io_debug_rb_inst_3_bits_vecWrites_7_bits_idx(
      io_debug_rb_inst_3_bits_vecWrites_7_bits_idx);
...
#define BIND_TESTBENCH_RB_INST(x)                                              \
  ...
  testbench.io_debug_rb_inst_##x##_bits_inst(                                  \
      io_debug_rb_inst_##x##_bits_inst);                                       \
  testbench.io_debug_rb_inst_##x##_bits_idx(io_debug_rb_inst_##x##_bits_idx);  \
  testbench.io_debug_rb_inst_##x##_bits_data(                                  \
      io_debug_rb_inst_##x##_bits_data);                                       \
  testbench.io_debug_rb_inst_##x##_bits_trap(                                  \
      io_debug_rb_inst_##x##_bits_trap);                                       \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_0_valid(                     \
      io_debug_rb_inst_##x##_bits_vecWrites_0_valid);                          \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_0_bits_data(                 \
      io_debug_rb_inst_##x##_bits_vecWrites_0_bits_data);                      \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_0_bits_idx(                  \
      io_debug_rb_inst_##x##_bits_vecWrites_0_bits_idx);                       \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_1_valid(                     \
      io_debug_rb_inst_##x##_bits_vecWrites_1_valid);                          \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_1_bits_data(                 \
      io_debug_rb_inst_##x##_bits_vecWrites_1_bits_data);                      \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_1_bits_idx(                  \
      io_debug_rb_inst_##x##_bits_vecWrites_1_bits_idx);                       \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_2_valid(                     \
      io_debug_rb_inst_##x##_bits_vecWrites_2_valid);                          \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_2_bits_data(                 \
      io_debug_rb_inst_##x##_bits_vecWrites_2_bits_data);                      \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_2_bits_idx(                  \
      io_debug_rb_inst_##x##_bits_vecWrites_2_bits_idx);                       \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_3_valid(                     \
      io_debug_rb_inst_##x##_bits_vecWrites_3_valid);                          \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_3_bits_data(                 \
      io_debug_rb_inst_##x##_bits_vecWrites_3_bits_data);                      \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_3_bits_idx(                  \
      io_debug_rb_inst_##x##_bits_vecWrites_3_bits_idx);                       \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_4_valid(                     \
      io_debug_rb_inst_##x##_bits_vecWrites_4_valid);                          \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_4_bits_data(                 \
      io_debug_rb_inst_##x##_bits_vecWrites_4_bits_data);                      \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_4_bits_idx(                  \
      io_debug_rb_inst_##x##_bits_vecWrites_4_bits_idx);                       \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_5_valid(                     \
      io_debug_rb_inst_##x##_bits_vecWrites_5_valid);                          \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_5_bits_data(                 \
      io_debug_rb_inst_##x##_bits_vecWrites_5_bits_data);                      \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_5_bits_idx(                  \
      io_debug_rb_inst_##x##_bits_vecWrites_5_bits_idx);                       \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_6_valid(                     \
      io_debug_rb_inst_##x##_bits_vecWrites_6_valid);                          \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_6_bits_data(                 \
      io_debug_rb_inst_##x##_bits_vecWrites_6_bits_data);                      \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_6_bits_idx(                  \
      io_debug_rb_inst_##x##_bits_vecWrites_6_bits_idx);                       \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_7_valid(                     \
      io_debug_rb_inst_##x##_bits_vecWrites_7_valid);                          \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_7_bits_data(                 \
      io_debug_rb_inst_##x##_bits_vecWrites_7_bits_data);                      \
  testbench.io_debug_rb_inst_##x##_bits_vecWrites_7_bits_idx(                  \
      io_debug_rb_inst_##x##_bits_vecWrites_7_bits_idx);
...
#undef INIT_CSR_IN
  ...
  if (testbench.instruction_count >= testbench.instruction_limit) {
    LOG(ERROR) << "Simulation TIMEOUT (Instruction count threshold reached: "
               << testbench.instruction_count << ").";
    return 1;
  }

  if (testbench.Cycle() >= testbench.cycle_limit) {
    LOG(ERROR) << "Simulation TIMEOUT (Cycle count threshold reached: "
               << testbench.Cycle() << ").";
    return 1;
  }
  ...
  LOG(ERROR) << "Simulation HANG detected (Internal SystemC error or "
                "unexpected state).";
  return 1;
}
...
int sc_main(int argc, char *argv[]) {
  ...
  return CoreRvviRun(SyscTb::GetName(const_cast<char *>(argv[0])), path,
