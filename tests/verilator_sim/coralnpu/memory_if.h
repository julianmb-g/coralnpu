...
public:
  ...
  MemoryIf(sc_module_name n, const char *bin, int limit,
  ...
  bool Read(uint64_t addr, void *dest, size_t count) {
    return Read(static_cast<uint32_t>(addr), static_cast<int>(count),
  ...
  bool Write(uint64_t addr, const void *src, size_t count) {
    return Write(static_cast<uint32_t>(addr), static_cast<int>(count),
  ...
  std::string GetProfileBounds() const {
    if (profile_ == "highmem") {
      return absl::StrFormat("[0x%08x - 0x%08x] (Unified ITCM)", itcm_base_,
                             itcm_base_ + itcm_size_);
    } else {
      return absl::StrFormat(
          "[0x%08x - 0x%08x] (ITCM) and [0x%08x - 0x%08x] (DTCM)", itcm_base_,
          itcm_base_ + itcm_size_, dtcm_base_, dtcm_base_ + dtcm_size_);
    }
  }

  // Helper to calculate the length of the intersection of two ranges [a1, a2)
  // and [b1, b2).
  static uint32_t GetRangeOverlap(uint32_t a1, uint32_t a2, uint32_t b1,
                                  uint32_t b2) {
  ...
  // Calculates the number of bytes by which an access at 'addr' with 'bytes'
  // exceeds bounds.
  uint32_t GetOverflowDelta(uint32_t addr, int bytes) const {
    uint32_t end_addr = addr + bytes;
    uint32_t itcm_overlap =
        GetRangeOverlap(addr, end_addr, itcm_base_, itcm_base_ + itcm_size_);
    uint32_t dtcm_overlap =
        GetRangeOverlap(addr, end_addr, dtcm_base_, dtcm_base_ + dtcm_size_);
