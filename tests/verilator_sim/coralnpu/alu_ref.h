...
template <typename T> typename std::make_unsigned<T>::type absd(T a, T b) {
...
template <typename Td, typename Ts> Td acc(Td a, Ts b) {
...
template <typename T> T add3(T a, T b, T c) {
  using UT = typename std::make_unsigned<T>::type;
  return static_cast<T>(static_cast<UT>(a) + static_cast<UT>(b) +
                        static_cast<UT>(c));
}
...
// Saturated addition.
template <typename T> T adds(T a, T b) {
...
// Widening add.
template <typename T> uint32_t addw(T a, T b) {
  if (std::is_signed<T>::value) {
...
template <typename T> T cmp_eq(T a, T b) { return a == b; }

template <typename T> T cmp_ne(T a, T b) { return a != b; }

template <typename T> T cmp_lt(T a, T b) { return a < b; }

template <typename T> T cmp_le(T a, T b) { return a <= b; }

template <typename T> T cmp_gt(T a, T b) { return a > b; }

template <typename T> T cmp_ge(T a, T b) { return a >= b; }

template <typename T> T dup(T b) { return b; }

template <typename T> T log_and(T a, T b) { return a & b; }

template <typename T> int log_clb(T x) {
...
template <typename T> int log_clz(const T x) {
  constexpr int n = sizeof(T) * 8;
  for (int count = 0; count < n; count++) {
    if ((x << count) >> (n - 1)) {
      return count;
...
template <typename T> int log_cpop(T a) {
  constexpr int n = sizeof(T) * 8;
  int count = 0;
  for (int i = 0; i < n; i++) {
    if (a & (1 << i)) {
...
template <typename T> T log_not(T a) { return ~a; }

template <typename T> T log_or(T a, T b) { return a | b; }

template <typename T> T log_rev(T a, T b) {
  T count = b & 0b11111;
  if (count & 1)
...
template <typename T> T log_ror(T a, T b) {
  if (sizeof(T) == 4) {
    if (b & 1)
      a = (a >> 1) | (a << 31);
    if (b & 2)
...
template <typename T> T log_xor(T a, T b) { return a ^ b; }

template <typename T> T hadd(T a, T b, int r) {
  if (std::is_signed<T>::value) {
...
template <typename T> T hsub(T a, T b, int r) {
  if (std::is_signed<T>::value) {
    return (static_cast<int64_t>(a) - static_cast<int64_t>(b) + r) >> 1;
  }
  return (static_cast<uint64_t>(a) - static_cast<uint64_t>(b) + r) >> 1;
...
template <typename T> T madd(T a, T b, T c) {
  if (std::is_signed<T>::value) {
...
template <typename T> T muls(T a, T b) {
  if (std::is_signed<T>::value) {
    int64_t m = static_cast<int64_t>(a) * static_cast<int64_t>(b);
    m = std::max(
...
template <typename T> uint32_t mulw(T a, T b) {
  if (std::is_signed<T>::value) {
    return static_cast<int64_t>(a) * static_cast<int64_t>(b);
  }
  return static_cast<uint64_t>(a) * static_cast<uint64_t>(b);
...
template <typename T> T mv(T a) { return a; }

template <typename T> std::pair<T, T> mvp(T a, T b) { return {a, b}; }

template <typename T> T dmulh(T a, T b, bool r, bool neg) {
  constexpr int n = sizeof(T) * 8;
...
template <typename T> T mulh(T a, T b, bool r) {
  constexpr int n = sizeof(T) * 8;
  if (std::is_signed<T>::value) {
    int64_t m = static_cast<int64_t>(a) * static_cast<int64_t>(b);
    m += r ? 1ll << (n - 1) : 0;
...
template <typename T> int32_t padd(T a, T b) {
  if (std::is_signed<T>::value) {
    return int64_t(a) + int64_t(b);
  }
  return uint64_t(a) + uint64_t(b);
...
template <typename T> uint32_t psub(T a, T b) {
  if (std::is_signed<T>::value) {
    return int64_t(a) - int64_t(b);
  }
  return uint64_t(a) - uint64_t(b);
...
template <typename T> T rsub(T a, T b) {
  using UT = typename std::make_unsigned<T>::type;
  return static_cast<T>(static_cast<UT>(b) - static_cast<UT>(a));
}

template <typename T> T shl(T a, T b) {
  constexpr int n = sizeof(T) * 8;
  b &= (n - 1);
  return a << b;
}

template <typename T> T shr(T a, T b) {
  constexpr int n = sizeof(T) * 8;
  b &= (n - 1);
  return a >> b;
}

template <typename T1, typename T2> T1 srans(T2 a, T1 b, bool r, bool u) {
...
template <typename T> T shf(T a, T b, bool r) {
...
template <typename T> T subs(T a, T b) {
  if (std::is_signed<T>::value) {
    int64_t m = static_cast<int64_t>(a) - static_cast<int64_t>(b);
    m = std::min<int64_t>(std::max<int64_t>(std::numeric_limits<T>::min(), m),
                          std::numeric_limits<T>::max());
...
template <typename T> uint32_t subw(T a, T b) {
  if (std::is_signed<T>::value) {
    return static_cast<int64_t>(a) - static_cast<int64_t>(b);
  }
  return static_cast<uint64_t>(a) - static_cast<uint64_t>(b);
