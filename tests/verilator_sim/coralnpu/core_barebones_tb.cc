...
#define CHECK_EBREAK(x) \
    if (io_debug_rb_inst_##x##_valid.read()) { \
        ...
        if (inst == 0x00100073 || inst == 0x9002 || (inst & 0xffff) == 0x9002) ebreak_halt = true; \
...
#undef BIND_DEBUG_DISPATCH
  ...
  if (memory_interface.PendingExitCode() != 0) {
    if (memory_interface.PendingExitCode() == 65) {
      LOG(ERROR) << absl::StrFormat("[FATAL] Runtime memory violation. Requested: [0x%08x - 0x%08x]. Available: %s. Delta: Exceeds bounds by 0x%08x bytes.",
