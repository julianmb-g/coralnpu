    ...
    entry_point = ::LoadElf(data8,
              [&mif, &load_ok](void* dest, const void* src, size_t count) {
                ...
                if (!mif.Write(addr, count, reinterpret_cast<const uint8_t*>(src))) {
                  std::string bounds = mif.GetProfileBounds();
                  ...
                  LOG(ERROR) << absl::StrFormat("[FATAL] ELF load violation. Requested: [0x%lx - 0x%lx]. Available: %s. Delta: Exceeds bounds by 0x%lx bytes.", addr, addr + count, bounds, delta);
                  load_ok = false;
                }
                return dest;
              });
